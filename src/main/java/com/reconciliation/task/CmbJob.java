...
public class CmbJob {
    ...
    private final static Logger logger = LoggerFactory.getLogger(CmbJob.class);
    ...
    public void cmbJob() throws  Exception{
                ...
                if (temp.get("isSearch").equals("1")) { //'停车场是否可读取数据库或FTP   0:否 1:是
                    if (temp.get("isFpt").toString().split(",")[1].equals("0")) {  //'区分供应商对账数据  0:读取fpt的txt   1:读取数据库',
                        ...
                        if (temp.get("parkId").toString().equals("1131")) {  //和义大道   只能单独获取代扣或主动缴费(需要分别调用接口)（参数：1.自助缴费 2.代扣成功）
                            ...
                            FileUtil.writeToFile(list,temp.get("parkId").toString(),temp.get("parkNameAbbreviation").toString());
